    ...
    def sample_action(self, db_outcomes, inv_offset=None):
        ...
        def apply_action_dropout_mask(action_dist, action_mask):
            if self.action_dropout_rate > 0:
                rand = torch.rand(action_dist.size(), device=action_dist.device)
            ...
            sample_action_dist = torch.nan_to_num(sample_action_dist, nan=0.0, posinf=0.0, neginf=0.0)
            sample_action_dist = sample_action_dist * action_mask
            row_sum = sample_action_dist.sum(dim=1, keepdim=True)
            uniform_dist = action_mask / (action_mask.sum(dim=1, keepdim=True) + ops.EPSILON)
            sample_action_dist = torch.where(row_sum <= 0, uniform_dist, sample_action_dist / (row_sum + ops.EPSILON))
        ...
        def sample(action_space, action_dist):
            ...
            action_prob = torch.clamp(action_prob, min=ops.EPSILON)
