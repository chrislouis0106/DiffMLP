...
# DiffMLP Diffusion Parameters
parser.add_argument('--diff_T_max', type=int, default=6,
                    help='Maximum diffusion steps for DiffMLP (default: 6)')
parser.add_argument('--diff_gamma', type=float, default=2.0,
                    help='Sensitivity for adaptive diffusion step control (default: 2.0)')
parser.add_argument('--diff_delta', type=float, default=0.9,
                    help='Early stopping cosine similarity threshold (default: 0.9)')
parser.add_argument('--diff_eta', type=float, default=0.01,
                    help='Prior constraint gradient step size (default: 0.01)')
parser.add_argument('--diff_vartheta', type=float, default=0.1,
                    help='Trade-off weight for semantic consistency loss L_g (default: 0.1)')
parser.add_argument('--diff_num_layers', type=int, default=2,
                    help='Number of GAT denoising layers (default: 2)')

# Path Reward Parameters (for improving MPS)
parser.add_argument('--use_path_reward', action='store_true',
                    help='Use path reward R_p in addition to entity reward R_e (default: False)')
parser.add_argument('--path_reward_weight', type=float, default=1.0,
                    help='Weight for path reward in R_K = R_e + λ·R_p (default: 1.0)')

# Tricks for improving prediction accuracy
parser.add_argument('--score_temperature', type=float, default=1.0,
                    help='Temperature for action score softmax (< 1.0 = sharper, > 1.0 = smoother) (default: 1.0)')
parser.add_argument('--topk_actions', type=int, default=0,
                    help='Keep only top-k actions before softmax (0 = disabled) (default: 0)')
parser.add_argument('--reward_shaping', action='store_true',
                    help='Add small intermediate reward for valid steps (default: False)')
parser.add_argument('--rule_bonus_weight', type=float, default=0.0,
                    help='Additive score bonus for actions that continue an AnyBURL rule prefix (default: 0.0)')
parser.add_argument('--no_op_penalty', type=float, default=0.0,
                    help='Penalty applied to NO_OP actions before the final hop (default: 0.0)')
parser.add_argument('--action_dist_epsilon', type=float, default=0.0,
                    help='Smoothing mass mixed into valid action probabilities (default: 0.0)')
parser.add_argument('--reward_clip', type=float, default=2.0,
                    help='Upper bound for combined reward before baseline normalization (default: 2.0)')
