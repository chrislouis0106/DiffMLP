...
class KnowledgeGraph(nn.Module):
    ...
    def __init__(self, args):
        ...
        if self.args.model.startswith('point') or self.args.model.startswith('diffmlp'):
            self.load_relation_attr(args.data_dir)
        self.store_eval_conf = {}
        self.rule_paths = {}
        if self.args.model.startswith('diffmlp'):
            self.load_rule_paths(args.data_dir)
        # Define NN Modules
    ...
    def load_rule_paths(self, data_dir):
        """
        Load AnyBURL-style relation rules for the DiffMLP conditional encoder.
        Expected format: {head_relation_id: [[confidence, [body_relation_ids...]], ...]}.
        """
        rule_path = os.path.join(data_dir, 'rules.dict')
        if not os.path.exists(rule_path):
            print('No AnyBURL rule file found at {}'.format(rule_path))
            return
        ...
        for head_r, rules in raw_rules.items():
            head_r = int(head_r)
            valid_rules = []
            shifted_rules = []
            for item in rules:
                if len(item) != 2:
                    continue
                conf, rel_path = item
                rel_path = [int(r) for r in rel_path if int(r) < num_relations]
                if rel_path:
                    valid_rules.append((float(conf), rel_path))
                shifted_path = [int(r) + 3 for r in rel_path if int(r) + 3 < num_relations]
                if shifted_path:
                    shifted_rules.append((float(conf), shifted_path))
            if valid_rules:
                self.rule_paths[head_r] = sorted(valid_rules, key=lambda x: x[0], reverse=True)
            if shifted_rules and head_r + 3 < num_relations:
                self.rule_paths.setdefault(
                    head_r + 3,
                    sorted(shifted_rules, key=lambda x: x[0], reverse=True))

        print('Sanity check: {} relations with AnyBURL rules loaded'.format(len(self.rule_paths)))
